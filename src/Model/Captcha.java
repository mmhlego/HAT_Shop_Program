...
public class Captcha extends AnchorPane {
	...
	public Captcha(int x, int y, int count) {
		this.setStyle("-fx-backgrond-color:#ffffff");
		this.x = x;
		this.y = y;
		this.count = count;
	...
	private void fontChange(Label t) {
		t.setFont(Font.font(r.nextInt(8) + 14));

	}

	private void textRotate(Label t) {
		t.setRotate((r.nextDouble() * 70) - 35);
	}

	private void textTranslate(Label t) {
		int boundX = r.nextInt((int) ((y) - (t.getFont().getSize())) - 10);
		int boundY = (int) (r.nextInt((int) ((x / count) - (2 * t.getWidth()))) + t.getWidth());
